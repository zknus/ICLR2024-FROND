...
class ODEFuncTransformerAtt_graphcon(ODEFunc):

  def __init__(self, in_features, out_features, opt, data, device):
    super(ODEFuncTransformerAtt_graphcon, self).__init__(opt, data, device)
  ...
  def forward(self, t, x_full):  # t is needed when called by the integrator
    ...
    self.nfe += 1
    attention, values = self.multihead_att_layer(y, self.edge_index)
    ay = self.multiply_attention(y, attention, values)
    ...
    if self.opt['add_source']:
      f = (1. - torch.sigmoid(self.beta_train)) * f + torch.sigmoid(self.beta_train) * self.x0[:,
                                                                                       self.opt['hidden_dim']:]
    f = torch.cat([f, (1. - torch.sigmoid(self.beta_train2)) * alpha * x + torch.sigmoid(self.beta_train2) * self.x0[:,
                                                                                                             :self.opt[
                                                                                                               'hidden_dim']]],
                  dim=1)
    return f
